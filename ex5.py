0# Задача 5. Запуск из командной строки
...
# Отдельный логгер для warnings
logger = logging.getLogger(__name__)
warnings = logging.FileHandler('warnings.log','w',encoding='utf-8')
formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')
warnings.setLevel(logging.WARNING)
warnings.setFormatter(formatter)
...
def collect_info(directory_path):
    ...
    # В файл warnings.log записываем все объекты в директории для проверки
        logger.warning(entry_path)
